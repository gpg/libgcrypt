...
do_twofish_setkey (gcry_core_context_t c,
		   TWOFISH_context *ctx, const byte *key, const unsigned keylen)
{
  int i, j, k;
...
static gcry_err_code_t
twofish_setkey (gcry_core_context_t c,
		void *context, const byte *key, unsigned int keylen)
{
  TWOFISH_context *ctx = context;
  int rc = do_twofish_setkey (c, ctx, key, keylen);
...
static void
twofish_encrypt (gcry_core_context_t c, void *context, byte *out, const byte *in)
{
  TWOFISH_context *ctx = context;
...
static void
twofish_decrypt (gcry_core_context_t c, void *context, byte *out, const byte *in)
{
  TWOFISH_context *ctx = context;
...
selftest (gcry_core_context_t c)
{
  ...
  twofish_setkey (c, &ctx, key, sizeof(key));
  twofish_encrypt (c, &ctx, scratch, plaintext);
  if (memcmp (scratch, ciphertext, sizeof (ciphertext)))
    return "Twofish-128 test encryption failed.";
  twofish_decrypt (c, &ctx, scratch, scratch);
  if (memcmp (scratch, plaintext, sizeof (plaintext)))
    return "Twofish-128 test decryption failed.";

  twofish_setkey (c, &ctx, key_256, sizeof(key_256));
  twofish_encrypt (c, &ctx, scratch, plaintext_256);
...
static struct gcry_core_cipher_spec gcry_core_cipher_twofish_struct =
  {
    "TWOFISH", NULL, NULL, 16, 256, sizeof (TWOFISH_context),
    twofish_setkey, twofish_encrypt, twofish_decrypt,
  };

gcry_core_cipher_spec_t gcry_core_cipher_twofish = &gcry_core_cipher_twofish_struct;

static struct gcry_core_cipher_spec gcry_core_cipher_twofish128_struct =
  {
    "TWOFISH128", NULL, NULL, 16, 128, sizeof (TWOFISH_context),
    twofish_setkey, twofish_encrypt, twofish_decrypt,
  };

gcry_core_cipher_spec_t gcry_core_cipher_twofish128 = &gcry_core_cipher_twofish128_struct;
