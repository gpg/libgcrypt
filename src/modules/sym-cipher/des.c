 ...
 * Copyright (C) 1998, 1999, 2001, 2002, 2003, 2005 Free Software Foundation, Inc.
 *
...
static int des_setkey (gcry_core_context_t ctx, struct _des_ctx *, const byte *);
static int des_ecb_crypt (struct _des_ctx *, const byte *, byte *, int);
static int tripledes_set2keys (gcry_core_context_t ctx,
			       struct _tripledes_ctx *,
                               const byte *, const byte *);
static int tripledes_set3keys (gcry_core_context_t ctx,
			       struct _tripledes_ctx *,
...
static const char *selftest (gcry_core_context_t ctx);
 ...
 */
static int
des_setkey (gcry_core_context_t c, struct _des_ctx *ctx, const byte * key)
{
 ...
 */
static int
tripledes_set2keys (gcry_core_context_t c,
		    struct _tripledes_ctx *ctx,
		    const byte * key1,
		    const byte * key2)
{
  int i;
 ...
 */
static int
tripledes_set3keys (gcry_core_context_t c,
		    struct _tripledes_ctx *ctx,
		    const byte * key1,
		    const byte * key2,
		    const byte * key3)
 ...
 */
static const char *
selftest (gcry_core_context_t ctx)
{
  /*
   ...
   */
  {
    ...
    for (i = 0; i < 16; ++i)
      {
	tripledes_set2keys (ctx, des3, key1, key2);
	tripledes_ecb_encrypt (des3, input, key1);
	tripledes_ecb_decrypt (des3, input, key2);
	tripledes_set3keys (ctx, des3, key1, input, key2);
  ...
  { 
    ...
    for (i=0; i<sizeof(testdata)/sizeof(*testdata); ++i)
      {
        tripledes_set3keys (ctx, des3, testdata[i].key,
...
do_tripledes_setkey (gcry_core_context_t c, void *context, const byte *key, unsigned keylen )
{
  ...
  tripledes_set3keys ( c, ctx, key, key+8, key+16);
...
static void
do_tripledes_encrypt(gcry_core_context_t c, void *context, byte *outbuf, const byte *inbuf )
{
  struct _tripledes_ctx *ctx = (struct _tripledes_ctx *) context;
...
static void
do_tripledes_decrypt(gcry_core_context_t c, void *context, byte *outbuf, const byte *inbuf )
{
  struct _tripledes_ctx *ctx = (struct _tripledes_ctx *) context;
...
static gcry_err_code_t
do_des_setkey (gcry_core_context_t c, void *context, const byte *key, unsigned keylen)
{
...
static void
do_des_encrypt(gcry_core_context_t c, void *context, byte *outbuf, const byte *inbuf )
{
  struct _des_ctx *ctx = (struct _des_ctx *) context;
...
static void
do_des_decrypt(gcry_core_context_t c, void *context, byte *outbuf, const byte *inbuf )
{
  struct _des_ctx *ctx = (struct _des_ctx *) context;
...
static struct gcry_core_cipher_spec gcry_core_cipher_des_struct =
  {
    "DES", NULL, NULL, 8, 64, sizeof (struct _des_ctx),
    do_des_setkey, do_des_encrypt, do_des_decrypt,
  };

gcry_core_cipher_spec_t gcry_core_cipher_des = &gcry_core_cipher_des_struct;

static gcry_core_cipher_oid_spec_t oids_tripledes[] =
  {
...
static struct gcry_core_cipher_spec gcry_core_cipher_tripledes_struct =
  {
    "3DES", NULL, oids_tripledes, 8, 192, sizeof (struct _tripledes_ctx),
    do_tripledes_setkey, do_tripledes_encrypt, do_tripledes_decrypt,
  };

gcry_core_cipher_spec_t gcry_core_cipher_tripledes = &gcry_core_cipher_tripledes_struct;
