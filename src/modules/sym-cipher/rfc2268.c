...
static const char *selftest (gcry_core_context_t c);


static void
do_encrypt (gcry_core_context_t c,
	    void *context, unsigned char *outbuf, const unsigned char *inbuf)
{
...
static void
do_decrypt (gcry_core_context_t c,
	    void *context, unsigned char *outbuf, const unsigned char *inbuf)
{
  RFC2268_context *ctx = context;
  register int i, j;
...
setkey_core (gcry_core_context_t c,
	     void *context, const unsigned char *key, unsigned int keylen, int with_phase2)
{
  ...
  if (!initialized)
    {
      ...
      if (selftest_failed)
        log_error (c, "RFC2268 selftest failed (%s).\n", selftest_failed);
    }
  if (selftest_failed)
...
static gpg_err_code_t
do_setkey (gcry_core_context_t ctx,
	   void *context, const unsigned char *key, unsigned int keylen)
{
  return setkey_core (ctx, context, key, keylen, 1);
}

static const char *
selftest (gcry_core_context_t c)
{
  RFC2268_context ctx;
  ...
  /* First test. */
  setkey_core (c, &ctx, key_1, sizeof(key_1), 0);
  do_encrypt (c, &ctx, scratch, plaintext_1);

  if (memcmp (scratch, ciphertext_1, sizeof(ciphertext_1)))
    return "RFC2268 encryption test 1 failed.";

  setkey_core (c, &ctx, key_1, sizeof(key_1), 0);
  do_decrypt (c, &ctx, scratch, scratch); 
  if (memcmp (scratch, plaintext_1, sizeof(plaintext_1)))
    return "RFC2268 decryption test 1 failed.";

  /* Second test. */
  setkey_core (c, &ctx, key_2, sizeof(key_2), 0);
  do_encrypt (c, &ctx, scratch, plaintext_2);
  ...
  /* Third test. */
  setkey_core(c, &ctx, key_3, sizeof(key_3), 0);
  do_encrypt(c, &ctx, scratch, plaintext_3);
...
static gcry_core_cipher_oid_spec_t oids_rfc2268_40[] =
  {
...
static struct gcry_core_cipher_spec gcry_core_cipher_rfc2268_40_struct = {
  "RFC2268_40", NULL, oids_rfc2268_40,
  RFC2268_BLOCKSIZE, 40, sizeof(RFC2268_context),
  do_setkey, do_encrypt, do_decrypt
};

gcry_core_cipher_spec_t gcry_core_cipher_rfc2268_40 = &gcry_core_cipher_rfc2268_40_struct;
