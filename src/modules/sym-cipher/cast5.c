...
static gcry_err_code_t cast_setkey (gcry_core_context_t ctx,
				    void *c, const byte *key, unsigned keylen);
static void encrypt_block (gcry_core_context_t ctx,
			   void *c, byte *outbuf, const byte *inbuf);
...
selftest(gcry_core_context_t ctx)
{
    ...
    cast_setkey(ctx, &c, key, 16 );
    encrypt_block(ctx, &c, buffer, plain );
    if( memcmp( buffer, cipher, 8 ) )
	return "1";
    decrypt_block(ctx, &c, buffer, buffer );
...
do_cast_setkey(gcry_core_context_t ctx, CAST5_context *c, const byte *key, unsigned keylen )
{
  ...
  if( !initialized ) 
    {
      ...
      if( selftest_failed )
        log_error (ctx, "CAST5 selftest failed (%s).\n", selftest_failed );
    }
  if( selftest_failed )
...
static gcry_err_code_t
cast_setkey (gcry_core_context_t ctx,
	     void *context, const byte *key, unsigned keylen )
{
  CAST5_context *c = (CAST5_context *) context;
  gcry_err_code_t rc = do_cast_setkey (ctx, c, key, keylen);
...
static struct gcry_core_cipher_spec gcry_core_cipher_cast5_struct =
  {
    "CAST5", NULL, NULL, CAST5_BLOCKSIZE, 128, sizeof (CAST5_context),
    cast_setkey, encrypt_block, decrypt_block,
  };

gcry_core_cipher_spec_t gcry_core_cipher_cast5 = &gcry_core_cipher_cast5_struct;
