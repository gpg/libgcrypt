		  ...
		  void *spec, gcry_module_t *module)
{
  ...
  if (! err)
    {
      entry = gcry_core_malloc (context, sizeof (struct gcry_module));
      if (! entry)
...
/* Lookup a module specification.  After a successfull lookup, the
   ...
   wanted module.  */
gcry_module_t 
_gcry_module_lookup (gcry_module_t entries, const void *data,
...
unsigned int
_gcry_module_last_reference_p (gcry_module_t module)
{
  return module->counter == 1;
}
