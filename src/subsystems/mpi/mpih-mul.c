 ...
 *               2001, 2002, 2005 Free Software Foundation, Inc.
 *
...
void
_gcry_mpih_mul_n( gcry_core_context_t ctx,  mpi_ptr_t prodp,
                     mpi_ptr_t up, mpi_ptr_t vp, mpi_size_t size)
{
	...
	if( size < KARATSUBA_THRESHOLD )
	    ...
	    mpi_ptr_t tspace;
	    secure = gcry_core_is_secure(ctx, up );
	    tspace = _gcry_mpi_alloc_limb_space(ctx, 2 * size, secure );
	    _gcry_mpih_sqr_n( prodp, up, size, tspace );
	    _gcry_mpi_free_limb_space (ctx, tspace, 2 * size );
	}
    }
    else {
	if( size < KARATSUBA_THRESHOLD )
	    ...
	    mpi_ptr_t tspace;
	    secure = gcry_core_is_secure(ctx, up ) || gcry_core_is_secure(ctx, vp );
	    tspace = _gcry_mpi_alloc_limb_space(ctx, 2 * size, secure );
	    mul_n (prodp, up, vp, size, tspace);
	    _gcry_mpi_free_limb_space (ctx, tspace, 2 * size );
	}
...
void
_gcry_mpih_mul_karatsuba_case( gcry_core_context_t context,  mpi_ptr_t prodp,
                                  ...
                                  struct karatsuba_ctx *ctx )
{
	...
	if( ctx->tspace )
	    _gcry_mpi_free_limb_space( context, ctx->tspace, ctx->tspace_nlimbs );
        ctx->tspace_nlimbs = 2 * vsize;
	ctx->tspace = _gcry_mpi_alloc_limb_space(context, 2 * vsize,
				            (gcry_core_is_secure(context, up )
                                            || gcry_core_is_secure(context, vp )) );
	...
	if( !ctx->tp || ctx->tp_size < vsize ) {
	    if( ctx->tp )
		_gcry_mpi_free_limb_space(context, ctx->tp, ctx->tp_nlimbs );
            ctx->tp_nlimbs = 2 * vsize;
	    ctx->tp = _gcry_mpi_alloc_limb_space(context, 2 * vsize, gcry_core_is_secure(context, up )
						      || gcry_core_is_secure(context, vp ) );
	    ctx->tp_size = vsize;
	}
	...
	if( usize < KARATSUBA_THRESHOLD ) {
	    _gcry_mpih_mul(context, ctx->tspace, vp, vsize, up, usize );
	}
	else {
	    if( !ctx->next ) {
		ctx->next = gcry_core_xcalloc( context, 1, sizeof *ctx );
	    }
	    _gcry_mpih_mul_karatsuba_case( context, ctx->tspace,
					vp, vsize,
					up, usize,
					ctx->next );
	}
...
void
_gcry_mpih_release_karatsuba_ctx(gcry_core_context_t context, struct karatsuba_ctx *ctx )
{
...
mpi_limb_t
_gcry_mpih_mul( gcry_core_context_t context, mpi_ptr_t prodp, mpi_ptr_t up, mpi_size_t usize,
                   mpi_ptr_t vp, mpi_size_t vsize)
{
    ...
    _gcry_mpih_mul_karatsuba_case( context, prodp, up, usize, vp, vsize, &ctx );
    _gcry_mpih_release_karatsuba_ctx(context, &ctx );
    return *prod_endp;
}
