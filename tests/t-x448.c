/* t-x448.c - Check the x448 crypto
 ...
 */
static void
test_x448 (int testno, const char *k_str, const char *u_str,
          ...
          const char *result_str)
{
  ...
  test_x448 (testno, a_priv_str, G_X, a_pub_str);
  /* Test B for private key corresponds to public key. */
  test_x448 (testno, b_priv_str, G_X, b_pub_str);
  /* Test DH with A's private key and B's public key. */
  test_x448 (testno, a_priv_str, b_pub_str, result_str);
  /* Test DH with B's private key and A's public key. */
  test_x448 (testno, b_priv_str, a_pub_str, result_str);
}
