/* t-rsa-931.c - Check the RSA X9.31 crypto
          ...
          const char *sha_alg, const char *msg, const char *s)
{
  ...
  err = gcry_pkey_open (&h0, GCRY_PKEY_RSA, flags, GCRY_PKEY_RSA_931, md_algo,
  ...
  flags &= ~GCRY_PKEY_FLAG_SECRET;
  err = gcry_pkey_open (&h1, GCRY_PKEY_RSA, flags, GCRY_PKEY_RSA_931, md_algo,
...
static void
check_rsa_931 (const char *fname)
{
  FILE *fp;
  int lineno, ntests;
  char *line;
...
main (int argc, char **argv)
{
  ...
  if (!fname)
    fname = prepend_srcdir ("t-rsa-931.inp");
  else
    custom_data_file = 1;
